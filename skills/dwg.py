...
class DwgSkill(Skill):
    ...
    def extract_walls_3d(self, path, output="", height=3.0):
        ...
        ok, err2 = self._exportar_step(segmentos, output_final, height, 0.15, "Muro")
        if not ok:
    ...
    def extract_all_layers_3d(self, path, output=""):
        ...
        lineas_script = [
            ...
            "        py = ux * GROSOR / 2.0",
            "        corners = [",
            "            FreeCAD.Vector(x1 + px, y1 + py, 0),",
            "            FreeCAD.Vector(x2 + px, y2 + py, 0),",
            "            FreeCAD.Vector(x2 - px, y2 - py, 0),",
            "            FreeCAD.Vector(x1 - px, y1 - py, 0),",
            "            FreeCAD.Vector(x1 + px, y1 + py, 0),",
            "        ]",
            "        wire = Part.makePolygon(corners)",
            "        face = Part.Face(wire)",
            "        solid = face.extrude(FreeCAD.Vector(0, 0, ALTURA))",
            "        obj = doc.addObject('Part::Feature', PREFIJO + str(idx))",
            "        obj.Shape = solid",
            "    except Exception as e:",
            "        print('WARN ' + str(idx) + ': ' + str(e))",
            "doc.recompute()",
    ...
    def _exportar_step(self, segmentos, output_path, altura, grosor, prefijo):
        ...
        lineas_script = [
            ...
            "        pl.Base = FreeCAD.Vector(x1, y1, 0)",
            "        pl.Rotation = FreeCAD.Rotation(FreeCAD.Vector(0, 0, 1), ang_rad)",
            "        # Aplicar la transformacion DIRECTAMENTE a la Shape",
            "        caja.transformShape(pl.toMatrix())",
            "        obj = doc.addObject('Part::Feature', PREFIJO + str(idx))",
            "        obj.Shape = caja",
            "    except Exception:",
            "        pass",
